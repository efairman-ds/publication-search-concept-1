...
/** The "Filter by: All publications" / "Publication date" row — three
 *  plain, static labels, not controls: no pill background, no caret, no
 *  tooltip, no menu, no click handler. Neither ever changes what's
 *  filtered or sorted any more; "All publications" still reflects
 *  `publicationFilter`'s actual value (so it can't silently go stale if
 *  that state is ever set some other way), it's just not clickable here.
 *  Still accepts the same props its one call site (SlimSearchBar) already
 *  passes; sortKey/sortDir/onSort/onPublicationFilterChange go unused
 *  now that there's nothing left to trigger from this row. */
function FilterControls({
  publicationFilter,
  onPublicationFilterChange: _onPublicationFilterChange,
  sortKey: _sortKey,
  sortDir: _sortDir,
  onSort: _onSort,
}: {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
      ...
      <Typography sx={{ fontSize: 14, fontWeight: 500, color: '#383f45', letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>
        {publicationFilter === 'all' ? 'All publications' : 'Clinical trials only'}
      </Typography>
      <Typography sx={{ fontSize: 14, fontWeight: 500, color: '#383f45', letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>
        Publication date
      </Typography>
...
/** One concept from the same mock ontology CONCEPT_DICTIONARY
...
interface DisplayConcept {
   ...
   *  matched. No longer read anywhere on this page (the "Edit concepts"
   *  UI that displayed it has been removed), but still set, since
   *  nothing writes it conditionally on whether it'll be shown. Left
   *  undefined (falsy) for
...
function computeConceptState(
  ...
  // A synonym promoted into `addedSynonyms` joins the same category as
  // its own removable chip — see synonymConceptKey/handleAddSynonym.
  // Its presence in `addedSynonyms` *is* the toggle: removing it deletes
  // it from that list (returning it to being offered again) rather than
  // adding to removedConceptKeys, which would have no way back — so
  // unlike every other push here, there's no separate removed state to
  // track. Shared by every concept source below (query matches *and*
  // concepts added via "Add concept") so both fold addedSynonyms in the
  // same way — though on this page, with the concept-editing UI removed
  // (see InterpretedConceptsBanner), addedSynonyms is never actually
  // populated any more; this stays purely as leftover, harmless
  // plumbing rather than something reachable.
  ...
  // customConcepts/addedSynonyms/removedConceptKeys have no UI writer any
  // more on this page (the "Edit concepts" entry point — the "Add
  // concept" search, the per-category green "+"/removable chips — has
  // been removed entirely; see InterpretedConceptsBanner), so this loop
  // and pushSynonymsFor below only ever run over an empty list in
  // practice. Left as-is rather than removed: it's harmless dead code,
  // and ripping it out would mean also touching removedConceptKeys/
  // addedSynonyms/customConcepts state and their handlers one level up,
  // which is a change to this page's underlying state, not to the
  // concept-editing UI this request is actually about.
...
/** The collapsed summary's read-only chip (ConceptSummaryChip) sets this
 *  exact height explicitly, rather than letting padding and content (a
 *  slightly larger label, an icon or not) settle it on their own. Text
 *  and icons still vertically centre within it (via `alignItems: center`
 *  on the chip's own flex root), so this is purely a floor/ceiling on
 *  the box itself. */
const CONCEPT_CHIP_HEIGHT = '28px';

/** One concept, display-only — no remove control, no click behaviour.
 *  This is the only way concepts are ever shown now: there's no editing
 *  entry point on the banner any more, so every concept — however many
 *  there are — renders as this same read-only chip. */
 ...
 *  concepts instead of carrying static copy.
 *
 *  A simple, scannable summary — every active concept as a plain,
 *  read-only chip, no category boxes, no remove controls, no
 *  add-a-concept search — so the banner never reads as a query builder.
 *  There is no editing entry point here any more (no "Edit concepts",
 *  no expanded category view) — this is display-only. */
function InterpretedConceptsBanner({
  query,
  concepts,
  onRemoveConcept: _onRemoveConcept,
  addedSynonyms: _addedSynonyms,
  onAddSynonym: _onAddSynonym,
  canUndo,
  onUndo,
  onAddCustomConcept: _onAddCustomConcept,
  combinationDrug,
  combinationScope,
  ...
  // (see DisplayConcept.removed) — the summary below only ever shows the
  // active ones; a removed concept simply disappears (Undo, in the
  // header, is how it comes back).
  ...
  return (
    <Box sx={{
      ...
      {activeConcepts.length === 0 ? (
        ...
        // Every active concept, flat — no category grouping, no
        // chip-level controls. maxHeight/overflow is a safety net
...
export default function PublicationSearchPage() {
  ...
  /** Promotes a synonym into its parent concept's category as its own
   *  active, removable chip, which computeConceptState folds into
   *  matchOrTerms on the next recompute. No longer reachable from any UI
   *  on this page (the concept-editing entry point that called this has
   *  been removed), but left in place rather than deleted along with
   *  that UI — see pushSynonymsFor's own comment. */
  ...
  /** Adds a manually-picked concept, which computeConceptState folds
   *  into its own real category (the option's own `category`) and
   *  matchOrTerms on the next recompute. No longer reachable from any UI
   *  on this page — the "Add concept" search that called this has been
   *  removed — but left in place rather than deleted; see
   *  pushSynonymsFor's own comment for why. */
  ...
  return (
    ...
    >
      ...
      <Box sx={{
        ...
        <Box sx={{ mt: showSlimLayout ? 0 : '220px', transition: 'margin-top 0.5s ease' }}>
